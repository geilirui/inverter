...
if __name__ == "__main__":
    ...
    checkpoint = ModelCheckpoint(log_dir + 'ep{epoch:03d}-loss{loss:.3f}-val_loss{val_loss:.3f}.h5',
        monitor='val_loss', save_weights_only=True, save_best_only=False, period=20)
    ...
    if False:
        ...
        if Cosine_scheduler:
            ...
            model.compile(optimizer=adam_v2.Adam(), loss={'yolo_loss': lambda y_true, y_pred: y_pred})
        else:
            reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=2, verbose=1)
            model.compile(optimizer=adam_v2.Adam(learning_rate_base), loss={'yolo_loss': lambda y_true, y_pred: y_pred})
